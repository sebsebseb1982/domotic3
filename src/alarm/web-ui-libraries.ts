export class WebUILibraries {
    getCredentials(userCode: string, modulePassword: string, ses: string) {
        let s_low = this.keeplowbyte(modulePassword);
        let temp = this.hex_md5(s_low);
        let spass = temp + ses;
        let p = this.hex_md5(spass);
        let u = this.rc4(spass, userCode);
